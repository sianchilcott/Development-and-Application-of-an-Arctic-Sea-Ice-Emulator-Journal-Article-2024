...
%% Fig A.4: Vary calibration parameters SIA
...
%% Fig A.2: Vary calibration parameters AMT
...
%% Fig A.6: MAGICC GMST ENSEMBLE WITH CMIP6 DATA 

colmat = [1,0,0; 0,0,1; 0,0.3906,0]; 
...
% Percentile: MAGICC 585
stdY_magicc = prctile(GT_ensembles{1}, [5 95]);

% Create curves: MAGICC
curve1 = stdY_magicc(2,:);
curve2 = stdY_magicc(1,:);


% Percentile: MAGICC 245
stdY_magicc_245 = prctile(GT_ensembles{2}, [5 95]);

% Create curves: MAGICC
curve1_245 = stdY_magicc_245(2,:);
curve2_245 = stdY_magicc_245(1,:);


% Percentile: MAGICC 126
stdY_magicc_126 = prctile(GT_ensembles{3}, [5 95]);

% Create curves: MAGICC
curve1_126 = stdY_magicc_126(2,:);
curve2_126 = stdY_magicc_126(1,:);


% Shading std area: MAGICC 585
x_magicc_hist = [1850:2015, fliplr(1850:2015)];
inBetween_magicc_hist = [curve1(1:166), fliplr(curve2(1:166))];


% Shading std area: MAGICC 585
x_magicc = [2015:2300, fliplr(2015:2300)];
inBetween_magicc = [curve1(166:end), fliplr(curve2(166:end))];


% Shading std area: MAGICC 245
x_magicc_245 = [2015:2300, fliplr(2015:2300)];
inBetween_magicc_245 = [curve1_245(166:end), fliplr(curve2_245(166:end))];


% Shading std area: MAGICC 126
x_magicc_126 = [2015:2300, fliplr(2015:2300)];
inBetween_magicc_126 = [curve1_126(166:end), fliplr(curve2_126(166:end))];


% Fill: MAGICC hist
c = [0 0 0];
f = fill(x_magicc_hist, inBetween_magicc_hist, c, 'edgecolor','none');
set(f,'facealpha',.3)
hold on

% Fill: MAGICC 585
c = colmat(1,:);
f = fill(x_magicc, inBetween_magicc, c, 'edgecolor','none');
set(f,'facealpha',.3)
hold on

% Fill: MAGICC 245
c = colmat(2,:);
f = fill(x_magicc_245, inBetween_magicc_245, c, 'edgecolor','none');
set(f,'facealpha',.3)
hold on

% Fill: MAGICC 126
c = colmat(3,:);
f = fill(x_magicc_126, inBetween_magicc_126, c, 'edgecolor','none');
set(f,'facealpha',.3)
hold on
...
% Percentile: CMIP6
stdY_cmip6 = prctile(cell2mat(tas_global{1}), [5 95]);

% Create curves: CMIP6
curve1_cmip6 = stdY_cmip6(2,:);
curve2_cmip6 = stdY_cmip6(1,:);



% plot means
plot(1850:2300, mean(GT_ensembles{1}), 'color', [1 0 0], 'linewidth', 2); hold on
plot(2015:2300, mean(GT_ensembles{2}(:,166:end)), 'color', [0 0 1], 'linewidth', 2); hold on
plot(2015:2300, mean(GT_ensembles{3}(:,166:end)), 'color', colmat(3,:), 'linewidth', 2); hold on
plot(1850:2015, mean(GT_ensembles{3}(:,1:166)), 'color', [0 0 0], 'linewidth', 2); hold on



plot(1850:2100, movmean(mean(cell2mat(tas_global{1})), 1), '--', 'color', [0 0 0], 'linewidth', 2); hold on
plot(1850:2100, curve1_cmip6, '-.', 'color', [0 0 0], 'linewidth', 1); hold on
plot(1850:2100, curve2_cmip6, '-.', 'color', [0 0 0], 'linewidth', 1); hold on


title('MAGICC Global Mean Temperature Ensemble', 'FontWeight', 'normal')
grid

h.Position = [0.3 0.2 0.55 0.55];

ylabel('Global Mean Temperature (\circC)')
xlabel('Year')
set(gca, 'Fontsize', 20)

tt2 = legend('Historical', 'SSP5-8.5', 'SSP2-4.5', 'SSP1-2.6', 'SSP5-8.5 Mean', 'SSP2-4.5 Mean', 'SSP1-2.6 Mean', 'Historical Mean', 'CMIP6 Mean', 'CMIP6 VLR', 'location', 'northwest', 'NumColumns', 2);
tt2.Box = 'off';


% tt2 = legend('Historical', 'SSP5-8.5', 'SSP2-4.5', 'SSP1-2.6', 'SSP5-8.5 Mean', 'SSP2-4.5 Mean', 'SSP1-2.6 Mean', 'Historical Mean', 'location', 'northwest', 'NumColumns', 2);
% tt2.Box = 'off';
...
MAGICC_ensemble_name_string = [];
counter = 0;
for ii = 1:600
    counter = counter+1;
    MAGICC_ensemble_name_string_hold = strcat("MAGICC Ensemble ", num2str(counter));
    MAGICC_ensemble_name_string = cat(1, MAGICC_ensemble_name_string, MAGICC_ensemble_name_string_hold);
end
...
%% SIA_max parameterisation comparison to CMIP6: Fig2.6, Chapter 2, Section 2.6
...
%% SENSITIVITY for completion seminar: sept co2 and GW
...
for month_ind = [9]
    ...
    obs_sens_co2 = (obs_sens_co2 .* 1e12) / 1e9;
    ...
    emul_cmip6_sens_co2 = cell2mat(emul_cmip6_sens_hold);
    emul_cmip6_sens_co2 = (emul_cmip6_sens_co2 .* 1e12) / 1e9;
    ...
    cmip6_sens_co2 = cell2mat(cmip6_sens_hold);
    cmip6_sens_co2 = (cmip6_sens_co2 .* 1e12) / 1e9;
...
% Add plot labels and edits
title([num2str(years(index_1974_2014(1))), '-', ...
num2str(years(index_1974_2014(end))), ' Sensitivity (dSIA/dGTA)'], 'fontsize', 28, 'interpreter', 'none')
...
length_curve_2 = 0.75;
xlim([0.25 length_curve_2])    
ylim([-6.3 0])    
xticks([0.5])
xticklabels(string({month_label{9}}));
grid
...
% Plot sensitivity as boxplots    
obs_sens_bp = boxplot(obs_sens_co2(:), 'positions', [ind-0.03], 'color', [0 0 0]+0.7, 'plotstyle', 'compact'); hold on
hAx = gca;
lines = hAx.Children;
lines = lines(1);
bp_cmip6 = findobj(lines, 'tag', 'Box');
bp_cmip6.YData(1,1) = quantile(obs_sens_co2(:), 1.0);
bp_cmip6.YData(1,2) = quantile(obs_sens_co2(:), 0.0);


boxplot(cmip6_sens_co2, 'positions', [ind], 'color', [1 0 0], 'plotstyle','compact'); hold on    % CMIP6
hAx = gca;
lines = hAx.Children;
lines = lines(1);
bp_cmip6 = findobj(lines, 'tag', 'Box');
bp_cmip6.YData(1,1) = quantile( cmip6_sens_co2, 0.25);
bp_cmip6.YData(1,2) = quantile( cmip6_sens_co2, 0.75);


boxplot(emul_cmip6_sens_co2, 'positions', [ind+0.03], 'color', colorss1(2,:), 'plotstyle','compact'); hold on    % CMIP6
hAx = gca;
lines = hAx.Children;
lines = lines(1);
bp_cmip6 = findobj(lines, 'tag', 'Box');
bp_cmip6.YData(1,1) = quantile( emul_cmip6_sens_co2, 0.25);
bp_cmip6.YData(1,2) = quantile( emul_cmip6_sens_co2, 0.75);
...
% Add plot labels and edits
title([num2str(years(index_1974_2014(1))), '-', ...
num2str(years(index_1974_2014(end))), ' Sensitivity (dSIA/dCO2)'], 'fontsize', 28, 'interpreter', 'none')
...
hLegend = legend(findall(gca,'Tag','Box'), {'CMIP6 Emulator', 'CMIP6', 'Plausible Range (from Observations)'}, 'location', 'southwest', 'fontsize', 18);
hLegend.Box = 'off';
...
% % Save Normalised
% temp=['Fig14_completion_seminar', '.pdf']; 
% saveas(gca,temp); 



%% Completion seminar sens
...
for month_ind = [9]
    ...
    boxplot(cmip6_sens, 'positions', [ind-0.03], 'color', [1 0 0], 'plotstyle','compact'); hold on    % CMIP6
    hAx = gca;
    lines = hAx.Children;
    lines = lines(1);
    bp_cmip6 = findobj(lines, 'tag', 'Box');
    ...
    SIE_constrained_bp = boxplot(magicc_sens_O1{1}, 'positions', [ind], 'color', colorss1(2,:), 'Labels', [' '], 'plotstyle','compact'); hold on % MAGICC using the UPPER BOUNDARY of our observationally constrained AA
    set(SIE_constrained_bp(length(SIE_constrained_bp),:),'Visible','off')     % Remove the outliers
    hAx = gca;
    lines = hAx.Children;
    lines = lines(1);
...
h.Position = [0.3 0.075 0.3 0.65];

% Add plot labels and edits
title([num2str(years(index_1974_2014(1))), '-', num2str(years(index_1974_2014(end))), ' Sensitivity (dSIA/dGTA)'],...
    'fontsize', 28, 'FontWeight', 'normal', 'interpreter', 'none')

grid 
hLegend = legend(findall(gca,'Tag','Box'), {'OC Emulator', 'CMIP6', 'Observations', 'Niederdrenk and Notz, (2018)'}, 'location', 'southwest', 'fontsize', 18);
hLegend.Box = 'off';
...
%% Fig2.8, Chapter 2, Section 2.4.1.1: MAGICC with CMIP6 AA and OBS CONSTRAINED AA
...
% Percentile: MAGICC 585
stdY_magicc = prctile(pi_new_emulation_global_annual_MCMC{1}, [5 95]);
...
% Percentile: MAGICC 245
stdY_magicc_CMIP6_AA = prctile(pi_new_emulation_global_annual_CMIP6_AA{1}, [5 95]);

% Create curves: MAGICC
curve1_cmip6_aa = stdY_magicc_CMIP6_AA(2,:);
curve2_cmip6_aa = stdY_magicc_CMIP6_AA(1,:);

% Shading std area: MAGICC 585
x_magicc = [1850:2300, fliplr(1850:2300)];
inBetween_magicc = [curve1, fliplr(curve2)];


% Shading std area: MAGICC 245
x_magicc_cmip6_aa = [1850:2300, fliplr(1850:2300)];
inBetween_magicc_cmip6_aa = [curve1_cmip6_aa, fliplr(curve2_cmip6_aa)];
...
% Fill: MAGICC 245
c = colmat(2,:);
f = fill(x_magicc_cmip6_aa, inBetween_magicc_cmip6_aa, c, 'edgecolor','none');
set(f,'facealpha',.3)
hold on

% % plot means
plot(1850:2300, mean(pi_new_emulation_global_annual_MCMC{1}), 'color', colmat(1,:), 'linewidth', 2); hold on
plot(1850:2300, mean(pi_new_emulation_global_annual_CMIP6_AA{1}), 'color', colmat(2,:), 'linewidth', 2); hold on
scatter(1850:2020, Arctic_annual_temp_obs, 'markerfacecolor', [0 0 0]+0.3, 'markeredgecolor', [0 0 0]+0.3); hold on

grid
h.Position = [0.3 0.2 0.55 0.55];
ylabel('Arctic Annual Mean Temperature (\circC)')
xlabel('Year')
set(gca, 'Fontsize', 20)
tt2 = legend('OC Emulator: SSP5-8.5 LR', 'OCCAA Emulator: SSP5-8.5 LR', 'OC Emulator: Mean', 'OCCAA Emulator: Mean', 'location', 'northwest', 'NumColumns', 2);
tt2.Box = 'off';
xlim([1850 2100])
set(gcf, 'PaperOrientation', 'landscape')
set(gcf,'PaperSize',[53 29.7000]); 
...
%% Fig2.12, Chapter 2, Section 2.4.2.2: SIA VS AMST to show need for bias corrections
...
%% Fig 2.13| Average 2080-2100 Calibrated AMST and SIA Annual Cycle
...
%% Fig2.15, Chapter 2, Section 2.5.3: SHOW UNCONSTRAINED, CONSTRAINED, OBS: FOR SECTION 3.2.1: Figure 8
...
%% Fig2.14, Chapter 2, Section 2.5.3: MAGICC SIA 1979-2014 Annual Curve: OUR EMULATION COMPARISON WITH OBSERVED
...
%%  Fig 2.11, Chapter 2, Section 2.4.2.2: Plot SIA and AMST bias correct and summer underestimation timeseries them together with observations
